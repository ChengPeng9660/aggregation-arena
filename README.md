...
先在 <https://app.tavily.com> 获取 Tavily Key。模型调用支持 Cloudflare AI Gateway，并用 Poe 保留 Cloudflare 目录缺失的精确型号。Cloudflare 路径使用 Worker 的 `AI` binding，不在 Worker 中保存 Cloudflare API token；Poe API Key 只进入 Cloudflare 加密 Secret。不要把任何 Key 写入 `wrangler.jsonc`、README、Git commit 或前端代码。
