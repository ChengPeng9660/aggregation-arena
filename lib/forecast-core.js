// The 20-question daily slate produces 320 forecasts across Aggrena's current
// 16-model Fixed Context panel. Sixteen jobs per hourly cron can drain a full
...
export const PROPHET_MODEL_PANEL_AS_OF = "2026-08-24";
...
export const FORECAST_MODELS = [
  ...
  {
    participantId: "prophet-qwen-3.7-plus",
    participantName: "Qwen 3.7 Plus",
    organization: "Qwen",
  ...
  {
    participantId: "prophet-inkling-256k",
    participantName: "Inkling 256K",
    organization: "Thinking Machines",
...
export function buildCloudflareBindingRequest(modelId, messages, options = {}) {
  const panelModelId = String(options.panelModelId || "").toLowerCase();
  const normalizedModelId = String(modelId).toLowerCase();
  ...
  if (normalizedModelId === "openai/gpt-5.6-sol" || panelModelId === "gpt-5.5-high") {
    ...
    return {
      ...
      ...(panelModelId === "gpt-5.5-high" ? { reasoning: { effort: "high" } } : {}),
    };
  }
  ...
  if (normalizedModelId.startsWith("anthropic/") || normalizedModelId.startsWith("thinkingmachines/")) {
    return {
      ...
      ...(system ? { system } : {}),
      ...(panelModelId === "claude-opus-4.8-thinking"
