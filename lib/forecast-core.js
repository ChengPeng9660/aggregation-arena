// A daily slate is measured in questions: every active model must forecast all
// twenty selected questions. Work is split into bounded hourly batches so a
// single scheduled Worker invocation never has to make the entire model panel.
export const DAILY_FORECAST_QUESTION_TARGET = 20;
export const FORECAST_JOBS_PER_BATCH = 20;
...
export function dailyForecastJobTarget(activeModelCount, questionTarget = DAILY_FORECAST_QUESTION_TARGET) {
  const models = Math.max(0, Math.floor(Number(activeModelCount) || 0));
  const questions = Math.max(0, Math.floor(Number(questionTarget) || 0));
