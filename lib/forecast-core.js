// The 13-model registry currently has 11 runnable models, producing up to 220
// forecasts per 20-question slate. Sixteen jobs per hourly cron can drain a full
...
export function modelGatewayRetryDelayMs(failedAttempt) {
  return [2000, 8000][Math.max(0, Math.min(1, Number(failedAttempt) || 0))];
}
