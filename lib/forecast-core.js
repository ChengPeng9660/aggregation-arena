// The 20-question daily slate produces 360 forecasts across Prophet Arena's
// 18-model Fixed Context panel. Sixteen jobs per hourly cron can drain a full
// daily release with a small retry margin.
export const FORECAST_JOBS_PER_RUN = 16;

// Pinned from Prophet Arena's public scoring API on 2026-08-10. Keep this date
// visible so a model-panel change is an explicit, auditable code change.
export const PROPHET_MODEL_PANEL_AS_OF = "2026-08-10";

export const FORECAST_MODELS = [
  {
    participantId: "prophet-gemini-3.6-flash",
    participantName: "Gemini 3.6 Flash",
    organization: "Google",
    modelId: "gemini-3.6-flash",
    promptVersion: "prophet-fixed-context-v1",
    color: "#4285f4",
  },
  {
    participantId: "prophet-claude-fable-5",
  ...
  {
    participantId: "prophet-gemini-3.1-pro",
  ...
  {
    participantId: "prophet-gpt-5.6-sol",
    participantName: "GPT-5.6 Sol",
    organization: "OpenAI",
    modelId: "gpt-5.6-sol",
    promptVersion: "prophet-fixed-context-v1",
    color: "#10a37f",
  ...
  {
    participantId: "prophet-gpt-5.5-high",
    participantName: "GPT-5.5",
    organization: "OpenAI",
  ...
  {
    participantId: "prophet-claude-opus-4.8-thinking",
  ...
  {
    participantId: "prophet-thinking-machines-zs-v2",
    participantName: "Inkling",
    organization: "Thinking Machines",
    modelId: "thinking-machines-zs-v2",
    promptVersion: "prophet-fixed-context-v1",
    color: "#7c3aed",
  },
  {
    participantId: "prophet-claude-sonnet-4.6",
  ...
  {
    participantId: "prophet-grok-4.5",
  ...
  {
    participantId: "prophet-deepseek-v4-pro",
  ...
  {
    participantId: "prophet-muse-spark-1.1",
    participantName: "Muse Spark 1.1",
    organization: "Meta",
  ...
  {
    participantId: "prophet-qwen-3.6-plus",
    participantName: "Qwen 3.6 Plus",
    organization: "Qwen",
  ...
  {
    participantId: "prophet-grok-4.3",
  ...
  {
    participantId: "prophet-inkling-small",
    participantName: "Inkling Small",
    organization: "Thinking Machines",
  ...
  {
    participantId: "prophet-minimax-m2.7",
  ...
  {
    participantId: "prophet-foresight-v3",
    participantName: "Foresight V3",
    organization: "Lightning Rod Labs",
    modelId: "foresight-v3",
    promptVersion: "prophet-fixed-context-v1",
    color: "#efab02",
  },
];
...
export function parseModelIdMap(value) {
  ...
  if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) {
    throw new Error("PROPHET_MODEL_ID_MAP must be a JSON object");
  }
  const entries = Object.entries(parsed).map(([slug, modelId]) => {
    if (!slug.trim() || typeof modelId !== "string" || !modelId.trim()) {
      throw new Error("PROPHET_MODEL_ID_MAP values must be non-empty strings");
    }
...
export function resolveGatewayModelId(modelId, modelIdMap) {
  return parseModelIdMap(modelIdMap)[modelId] || modelId;
}
...
export function buildGatewayRequest(modelId, messages, options = {}) {
  return {
    model: modelId,
    messages,
...
export function buildGatewayRequestForEndpoint(endpoint, modelId, messages, options = {}) {
  const payload = buildGatewayRequest(modelId, messages, options);
  let hostname = "";
  let pathname = "";
  try {
    const parsedEndpoint = new URL(endpoint);
    hostname = parsedEndpoint.hostname.toLowerCase();
    pathname = parsedEndpoint.pathname.toLowerCase();
  } catch {
    return payload;
  }
  if (hostname === "api.poe.com" || hostname.endsWith(".api.poe.com")) {
    // Poe documents these fields as ignored, but some provider-backed public
    // bots reject them as unsupported extra_body values. The prompts already
    // require JSON-only output and the parser validates the result strictly.
    delete payload.seed;
    delete payload.response_format;
    // Reasoning tokens count against the completion ceiling on Poe. Several
    // current frontier bots can spend ~1.3k tokens reasoning before emitting
    // the short JSON answer, so the old 500/700-token ceilings truncated valid
    // responses. This is a ceiling, not a requested output length.
    const normalizedModelId = String(modelId).toLowerCase();
    const heavyDeliberativeBots = new Set(["deepseek-v4-pro", "minimax-m2.7"]);
    const deliberativeBots = new Set(["inkling"]);
    payload.max_tokens = Math.max(
      payload.max_tokens,
      heavyDeliberativeBots.has(normalizedModelId) ? 12000 : deliberativeBots.has(normalizedModelId) ? 6000 : 2200,
    );
