 ...
 * }>}
 */
export function buildBestPairStandings({ events, methods, participants = [], minimumResolved = 5 }) {
  ...
  return methods.flatMap((method) => {
    ...
    const sufficientlyResolved = candidates.filter((candidate) => candidate.losses.length >= minimumResolved);
    const selectionPool = sufficientlyResolved.length ? sufficientlyResolved : candidates;
