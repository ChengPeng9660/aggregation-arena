...
export function pipelineReportedFailure(result) {
  ...
  const readySelection = value.selection && typeof value.selection === 'object'
    && 'quotaMet' in value.selection && value.selection.quotaMet === true;
  if (resolution && typeof resolution === 'object' && 'failed' in resolution && Number(resolution.failed) > 0) {
    return `${resolution.failed} selected market resolution checks failed; see source diagnostics`;
  }
  // A provider outage remains visible in sourceStats/automation, but is no
  // longer a failed daily release when the approved fallback filled all twenty.
  if (!readySelection && sync && typeof sync === 'object' && 'sourceStats' in sync && sync.sourceStats && typeof sync.sourceStats === 'object') {
  ...
  if (selection && typeof selection === 'object' && 'quotaMet' in selection && selection.quotaMet === false) {
    return 'Daily selection did not satisfy the required 20 questions and diversity checks';
  }
