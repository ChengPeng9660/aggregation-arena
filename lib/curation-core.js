import { selectStrictBalancedSlate, selectAvailableSlate } from "./curation-selection-core.js";
...
export const CURATION_CONFIG = {
  configVersion: "dual-market-20-v3-availability-first",
  balancePolicy: "preferred",
  ...
  fallbackMinimumVolume24h: 1_000,
...
export function dailySelectionRunId(now = new Date()) {
  return `live-${now.toISOString().slice(0, 10)}-${CURATION_CONFIG.configVersion}`;
}
...
export function classifyMarket(event, market) {
  ...
  // Specific model/research questions outrank generic words such as "company".
  // Keep financial outcomes (e.g. an OpenAI IPO) in Economics.
  if (/\b(ai model|language model|model release|mythos[- ]class|scientific benchmark)\b/i.test(text)
    && !/\b(ipo|revenue|earnings|stock|market cap|acquisition)\b/i.test(text)) {
    return { category: "Science", confidence: 0.95 };
  }
...
export function evaluateDailyEligibility(candidate, now = new Date()) {
  const check = evaluateHardEligibility(candidate, now);
  const fallbackEligible = !check.eligible && candidate.sourcePlatform === "polymarket"
    && candidate.volume24h >= CURATION_CONFIG.fallbackMinimumVolume24h
    && check.reasons.length === 1 && check.reasons[0] === "low_24h_volume";
  // Preserve the regular rejection reason for public/audit provenance.
  return { ...check, eligible: check.eligible || fallbackEligible,
    regularEligible: check.eligible, fallbackEligible };
}

export function rankDailyCandidates(candidates, now = new Date()) {
  return rankCandidates(candidates, now).map(candidate => ({
    ...candidate, ...evaluateDailyEligibility(candidate, now),
  }));
}
...
export function selectDiverseSourceBalancedCandidates(candidates, options = {}) {
  const recentCounts = options.recentCategoryCounts || {};
  const categories = [...CANONICAL_CATEGORIES].sort((a, b) =>
  ...
  return selectStrictBalancedSlate(candidates, {
    categories,
    sourceQuotas: options.sourceQuotas || CURATION_CONFIG.sourceQuotas,
    categoryTarget: options.targetPerCategory || CURATION_CONFIG.targetPerCategory,
    sourceCategoryTarget: options.sourceTargetPerCategory || CURATION_CONFIG.sourceTargetPerCategory,
    blockedGroups: options.blockedDiversityGroupIds || [],
    recentTitles: options.recentTitles || [],
    conflicts: (left, right) => titlesConflict(left, right, similarityThreshold),
  });
}

export function selectDailyAvailableCandidates(candidates, options = {}) {
  const regular = candidates.filter(candidate => candidate.regularEligible !== false);
  const balanced = selectDiverseSourceBalancedCandidates(regular, options);
  if (validateDailySlate(balanced).valid) return balanced;
  const settings = {
    categories: CANONICAL_CATEGORIES, sourceQuotas: CURATION_CONFIG.sourceQuotas,
    target: CURATION_CONFIG.dailyTotal, categoryTarget: CURATION_CONFIG.targetPerCategory,
    blockedGroups: options.blockedDiversityGroupIds || [], recentTitles: options.recentTitles || [],
    recentCategoryCounts: options.recentCategoryCounts || {},
    conflicts: (left, right) => titlesConflict(left, right, CURATION_CONFIG.titleSimilarityThreshold),
  };
  const regularSlate = selectAvailableSlate(regular, settings);
  if (regularSlate.length === CURATION_CONFIG.dailyTotal) return regularSlate;
  return selectAvailableSlate(candidates, settings);
}
...
export function validateDailySlate(selected, options = {}) {
  ...
  return {
    valid: dailyTotalMet && categoriesKnown && uniqueDiversityGroups && uniqueTitles
      && (options.requireBalance === false || (sourceQuotasMet && categoryQuotasMet)),
    dailyTotalMet,
    sourceQuotasMet,
    categoryQuotasMet,
...
export function diversityAnchors(value) {
  const ignored = new Set([
    ...
    "us", "usa", "united", "states", "kingdom", "health", "effective", "winner",
