...
export const CURATION_CONFIG = {
  configVersion: "polymarket-event-slate-v2",
...
const CATEGORY_RULES = [
  ...
  ["Finance & Crypto", /\b(bitcoin|btc|ethereum|eth|crypto|token|solana|xrp|stock|s&p|nasdaq|dow|etf|fed funds|interest rate|market cap|ipo)\b/i],
  ["Economics", /\b(gdp|inflation|cpi|unemployment|recession|tariff|trade deficit|jobs report|payroll|economic growth|central bank)\b/i],
  ["Science & Health", /\b(fda|drug|vaccine|disease|health|clinical trial|science|space|nasa|climate|temperature|ai safety|research)\b/i],
  ["Business & Technology", /\b(apple|google|meta|microsoft|amazon|openai|anthropic|nvidia|tesla|company|ceo|acquisition|merger|technology|artificial intelligence|ai model)\b/i],
  ["Politics & Policy", /\b(election|president|prime minister|congress|senate|governor|vote|poll|cabinet|parliament|supreme court|law|policy|war|ceasefire|sanction|trump|democrat|republican)\b/i],
  ["Culture & World", /\b(oscar|grammy|movie|film|music|celebrity|award|streaming|box office|culture|pope|royal|world)\b/i],
];
...
export function classifyMarket(event, market) {
  ...
  return { category: "Culture & World", confidence: 0.45 };
}
...
export function rankCandidates(candidates, now = new Date()) {
  ...
  return evaluated.map((candidate) => {
    ...
    const selectionScore =
      ...
      0.10 * closeQuality;
    const percentileEligible = (candidate.volume24Percentile || 0) >= CURATION_CONFIG.minimumCategoryPercentile;
    return {
      ...candidate,
      selectionScore,
      eligible: candidate.eligible && percentileEligible,
      reasons: candidate.eligible && !percentileEligible ? [...candidate.reasons, "below_category_volume_percentile"] : candidate.reasons,
    };
  });
}
