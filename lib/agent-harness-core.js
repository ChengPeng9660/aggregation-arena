...
export function buildHarnessPrompt(snapshot) {
  ...
  return `You are a forecast aggregation router. Choose one convex pool over the supplied forecasters that is intended to minimize the expected Prophet Event Brier score for this event.
...
DECISION PROCEDURE
- Inspect the forecast distribution: agreement, disagreement, clusters, and isolated forecasts.
- Treat historical Brier estimates according to their sample support. Prefer better supported performance, but do not overfit small or tightly clustered histories.
- Account for redundancy: many similar forecasts are not automatically independent evidence, and an isolated forecast is not automatically superior.
${evidenceAware
    ? "- Assess whether each anonymous rationale is specific, internally coherent, consistent with the frozen event rules and evidence, and appropriately calibrated. Do not reward verbosity or confident wording by itself."
    : "- With no event content available, base any departure from uniform pooling only on the anonymous forecasts and strictly pre-event performance summaries."}
- Use the simplest allocation justified by the supplied information. Uniform weights are permitted when the forecasters are genuinely indistinguishable; do not choose them merely as a formatting default. Any unequal allocation must be supported by the input.
- Calibrate the strength of weight differences to the strength of the evidence. Avoid unsupported zero or near-zero weights.

OUTPUT CONTRACT
- Return one JSON object and no surrounding prose or markdown.
- The top-level keys must be weights, rationale, and confidence.
- weights must be an object containing every allowed alias exactly once and no other aliases. Values must be finite non-negative decimal numbers with a strictly positive total; make them sum to 1.
- rationale must be one concise sentence identifying the main input signal behind the allocation without revealing hidden reasoning.
- confidence must be one of low, medium, or high and must reflect confidence in the relative weights, not confidence in the event outcome.
- Allowed aliases: ${allowed.join(", ")}.

INPUT
${JSON.stringify(input)}

Return the contracted JSON object now.`;
}
...
function responseText(raw) {
  ...
  if (Array.isArray(raw?.content)) {
    return raw.content.filter((item) => item?.type === "text" && typeof item?.text === "string")
      .map((item) => item.text)
      .join("\n");
  }
  if (Array.isArray(raw?.candidates?.[0]?.content?.parts)) {
    return raw.candidates[0].content.parts.filter((item) => typeof item?.text === "string")
      .map((item) => item.text)
      .join("\n");
  }
  ...
  if (Array.isArray(raw?.output)) {
    return raw.output.flatMap((item) => Array.isArray(item?.content) ? item.content : [])
