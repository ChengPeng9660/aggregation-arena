...
export function selectStrictBalancedSlate(candidates, {
  ...
  function search(available) {
    ...
    const choices = [...byCategory.get(category)].sort((a, b) =>
      // Two per provider/category remains a preference, never a reason to
      // violate the hard four-per-category or ten-per-provider requirements.
    ...
    for (const index of choices) {
      ...
      const result = search(valid.filter((other) => other !== index && compatible(index, other)));
