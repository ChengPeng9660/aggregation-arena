...
const DEFAULT_OUTPUT = "output/research/frontier-rescue-dash-v3-discovery-2026-08-23.json";
...
const V3_OVERALL_ID = "frontier-rescue-joint-d1-n1000-m0p0005-b0p375";
const V3_BALANCED_ID = "frontier-rescue-joint-d1-n1000-m0p00015-b0p25";
const V3_COVERAGE_ID = "frontier-rescue-coverage-d1-n1000-m0p00015-b0p125";
const V3_STRONG_ID = "frontier-rescue-strong-d1-n1000-m0p00015-b0p5";
...
function conservativeRescuePrediction(predictions, state, minFeedback, margin, blend) {
  if (predictions.length !== 5 || state.expertLossSum.length !== 5) {
    throw new Error("Conservative rescue requires one V2 candidate and four incumbent baselines");
  }
  if (state.nFeedback < minFeedback) return predictions[0];
  const averageLoss = state.expertLossSum.map((loss) => loss / Math.max(1, state.nFeedback));
  const baselineIndex = 1 + minimumIndex(averageLoss.slice(1));
  const baselineAdvantage = averageLoss[0] - averageLoss[baselineIndex];
  if (baselineAdvantage <= margin) return predictions[0];
  return (1 - blend) * predictions[0] + blend * predictions[baselineIndex];
}

function rescueFeatureVector(cell) {
  const [modelA, modelB] = cell.pair.split("\u0000");
  return [
    10 * cell.priorQuality,
    20 * cell.priorQualityGap,
    2 * cell.safeAlpha,
    Math.log1p(cell.nHistory) / 8,
    modelProvider(modelA) === modelProvider(modelB) ? 0.25 : 0,
  ];
}
...
function knnRescueSelection(cell, examples, config) {
  if (!examples.length) return 0;
  const features = rescueFeatureVector(cell);
  const neighbors = examples
    .map((example) => ({ example, distance: squaredDistance(features, example.features) }))
    .sort((first, second) => first.distance - second.distance)
    .slice(0, Math.min(config.neighbors, examples.length));
  const loss = Array(config.experts.length).fill(0);
  let targets = 0;
  for (const { example } of neighbors) {
    for (let index = 0; index < loss.length; index += 1) loss[index] += example.loss[index];
    targets += example.targets;
  }
  const averageLoss = loss.map((value) => value / targets);
  const baselineIndex = 1 + minimumIndex(averageLoss.slice(1));
  return averageLoss[0] - averageLoss[baselineIndex] > config.margin ? baselineIndex : 0;
}
...
function pairStackConfigs() {
  ...
  for (const [anchorName, anchor] of [
    ["joint", FRONTIER_JOINT_ID],
    ["coverage", FRONTIER_COVERAGE_ID],
    ["strong", FRONTIER_STRONG_ID],
  ]) {
    const rescueExperts = [anchor, ...SOTA_BASELINES];
    for (const margin of [0, 0.00005, 0.0001, 0.00015, 0.00025, 0.0005]) {
      for (const blend of [0.125, 0.25, 0.375, 0.5]) {
        configs.push({
          id: `frontier-rescue-${anchorName}-d1-n1000-m${formatParameter(margin)}-b${formatParameter(blend)}`,
          family: "pair-specific-conservative-frontier-rescue",
          rule: "rescue",
          discount: 1,
          minFeedback: 1000,
          margin,
          blend,
          experts: rescueExperts,
        });
      }
    }
  }
  for (const [anchorName, anchor] of [
    ...
    for (const minFeedback of [1000, 5000]) {
      for (const margin of [0.00025, 0.0005]) {
        for (const blend of [0.25, 0.5]) {
          const componentIds = [];
          for (const scope of ["model-a", "model-b"]) {
            const id = `frontier-model-rescue-${anchorName}-${scope}-n${minFeedback}-m${formatParameter(margin)}-b${formatParameter(blend)}`;
            componentIds.push(id);
            configs.push({
              id,
              family: "model-specific-conservative-frontier-rescue",
              rule: "rescue",
              scope,
              discount: 1,
              minFeedback,
              margin,
              blend,
              experts: [anchor, ...SOTA_BASELINES],
            });
          }
          configs.push({
            id: `frontier-model-rescue-${anchorName}-symmetric-n${minFeedback}-m${formatParameter(margin)}-b${formatParameter(blend)}`,
            family: "symmetric-model-specific-frontier-rescue",
            rule: "fixed",
            discount: 1,
            experts: componentIds,
            prior: [0.5, 0.5],
          });
        }
      }
...
function knnRescueConfigs() {
  ...
  for (const [anchorName, anchor] of [
    ...
    for (const neighbors of [25, 50, 100]) {
      for (const margin of [0, 0.00025, 0.0005]) {
        for (const blend of [0.25, 0.5, 1]) {
          configs.push({
            id: `frontier-knn-${anchorName}-k${neighbors}-m${formatParameter(margin)}-b${formatParameter(blend)}`,
            family: "strictly-prior-knn-frontier-rescue",
            anchor,
            neighbors,
            margin,
            blend,
...
function evaluateStrategies(cellsByDate) {
  ...
  const knnRescues = knnRescueConfigs();
  ...
  const knnHistoryByAnchor = new Map([
    FRONTIER_JOINT_ID,
    FRONTIER_COVERAGE_ID,
    FRONTIER_STRONG_ID,
  ].map((anchor) => [anchor, []]));
  let allKnnExamplesStrictlyPrior = true;
  ...
  for (const date of dates) {
    ...
    const knnSelectionCache = new Map();
    const knnSelections = new Map();
    for (const config of knnRescues) {
      const cacheKey = `${config.anchor}\u0000${config.neighbors}\u0000${config.margin}`;
      if (!knnSelectionCache.has(cacheKey)) {
        const examples = knnHistoryByAnchor.get(config.anchor);
        if (examples.some((example) => example.date >= date)) allKnnExamplesStrictlyPrior = false;
        knnSelectionCache.set(cacheKey, new Map(cells.map((cell) => [
          cell.id,
          knnRescueSelection(cell, examples, config),
        ])));
      }
      knnSelections.set(config.id, knnSelectionCache.get(cacheKey));
    }
    for (const cell of cells) {
    ...
    const knnAnchorUpdates = new Map([...knnHistoryByAnchor.keys()].map((anchor) => [anchor, new Map()]));
    ...
    for (const cell of cells) {
      ...
      for (const row of cell.rows) {
        ...
        for (const [anchor, anchorUpdates] of knnAnchorUpdates) {
          if (!anchorUpdates.has(cell.id)) {
            anchorUpdates.set(cell.id, {
              date,
              features: rescueFeatureVector(cell),
              targets: 0,
              loss: Array(1 + SOTA_BASELINES.length).fill(0),
            });
          }
          const update = anchorUpdates.get(cell.id);
          const vector = [anchor, ...SOTA_BASELINES].map((expert) => predictions[expert]);
          update.targets += 1;
          for (let index = 0; index < vector.length; index += 1) {
            update.loss[index] += brier(vector[index], row.outcome);
          }
        }
        for (const config of knnRescues) {
          const vector = config.experts.map((expert) => predictions[expert]);
          const selectedIndex = knnSelections.get(config.id).get(cell.id);
          predictions[config.id] = selectedIndex === 0
            ? vector[0]
            : (1 - config.blend) * vector[0] + config.blend * vector[selectedIndex];
        }
    ...
    for (const [anchor, anchorUpdates] of knnAnchorUpdates) {
      knnHistoryByAnchor.get(anchor).push(...anchorUpdates.values());
    }
  ...
  const frontierV3TrailingWindow = Object.fromEntries([3, 5, 8].map((windowSize) => {
    const windowDates = dates.slice(-windowSize);
    const windowDateSet = new Set(windowDates);
    const overallDates = subsetAggregates(dateAggregates, windowDateSet);
    const q1Dates = subsetAggregates(q1DateAggregates, windowDateSet);
  ...
  return {
    ...
    frontierV3Recommendation: {
      overallMean: candidatesById.get(V3_OVERALL_ID),
      jointOverallAndStrongestGroup: candidatesById.get(V3_BALANCED_ID),
      strongestGroupCoverage: candidatesById.get(V3_COVERAGE_ID),
      strongestGroupMean: candidatesById.get(V3_STRONG_ID),
      overallVsV2DateBootstrap: dateBlockBootstrap(dateAggregates, FRONTIER_JOINT_ID, V3_OVERALL_ID, 20_000, 20_261_001),
      balancedOverallVsV2DateBootstrap: dateBlockBootstrap(dateAggregates, FRONTIER_JOINT_ID, V3_BALANCED_ID, 20_000, 20_261_002),
      balancedStrongestQ1VsV2DateBootstrap: dateBlockBootstrap(q1DateAggregates, FRONTIER_JOINT_ID, V3_BALANCED_ID, 20_000, 20_261_003),
      coverageOverallVsV2DateBootstrap: dateBlockBootstrap(dateAggregates, FRONTIER_COVERAGE_ID, V3_COVERAGE_ID, 20_000, 20_261_004),
      coverageStrongestQ1VsV2DateBootstrap: dateBlockBootstrap(q1DateAggregates, FRONTIER_COVERAGE_ID, V3_COVERAGE_ID, 20_000, 20_261_005),
      strongestQ1VsV2DateBootstrap: dateBlockBootstrap(q1DateAggregates, FRONTIER_STRONG_ID, V3_STRONG_ID, 20_000, 20_261_006),
      overallPairSotaTransitionVsV2: sotaTransition(pairAggregates, FRONTIER_JOINT_ID, V3_OVERALL_ID),
      balancedPairSotaTransitionVsV2: sotaTransition(pairAggregates, FRONTIER_JOINT_ID, V3_BALANCED_ID),
      balancedStrongestQ1SotaTransitionVsV2: sotaTransition(q1PairAggregates, FRONTIER_JOINT_ID, V3_BALANCED_ID),
      coverageStrongestQ1SotaTransitionVsV2: sotaTransition(q1PairAggregates, FRONTIER_COVERAGE_ID, V3_COVERAGE_ID),
      strongestQ1SotaTransitionVsV2: sotaTransition(q1PairAggregates, FRONTIER_STRONG_ID, V3_STRONG_ID),
      negativeFamilies: {
        knnFrontierRescue: "did not improve the 101/118 strongest-Q1 coverage ceiling and did not create a new mean Pareto point",
        modelSpecificRescue: "symmetric endpoint states did not improve the 101/118 strongest-Q1 coverage ceiling or the direct pair-rescue Pareto frontier",
      },
      candidateSearchWarning: "Selected on the same historical replay after a coarse and local refinement search; discovery evidence only until frozen and evaluated on future dates.",
    },
    frontierV3TrailingWindow,
...
async function main() {
  ...
  const result = {
    schemaVersion: "0.9.0-frontier-rescue-discovery",
    ...
    protocol: {
      ...
      knnFrontierRescue: "nearest-neighbor rescue uses pair-date examples from strictly earlier dates only; current-date selections are frozen before any current-date outcome is appended",
    ...
    audit: {
      ...
      allKnnExamplesStrictlyPrior: summary.allKnnExamplesStrictlyPrior,
    },
    expertNames: {
